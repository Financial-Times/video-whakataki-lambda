...
gulp.task('deploy:cloudformation', function(callback) {
    var cloudformation_template = 'src/main/cloudformation/' + gutil.env.env + '/flex-cloud-formation-whakataki.json';
...
gulp.task('deploy:lambda-zip', function() {
    gulp.src('./target/whakataki-concept-api-lambda-1.0-SNAPSHOT.jar')
        .pipe(zip('whakataki-concept-api-lambda.zip'))
        .pipe(gulp.dest('.'));
});

gulp.task('deploy:lambda-upload', function() {
    //gulp.src('whakataki-concept-api-lambda.zip')
    gulp.src('./target/whakataki-concept-api-lambda-1.0-SNAPSHOT.jar')
        .pipe(s3({
...
gulp.task('deploy:cloudformation-template', function() {
    ...
    gulp.src('src/main/cloudformation/flex-cloud-formation-whakataki.json')
        ...
        .pipe(gulp.dest('src/main/cloudformation/' + gutil.env.env));
});

gulp.task('deploy', function(callback) {
    sequence('deploy:cloudformation-template', 'deploy:lambda-upload', 'deploy:cloudformation', callback);
});
